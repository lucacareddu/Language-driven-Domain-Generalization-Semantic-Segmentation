...
class Compose(object):
    ...
    def __call__(self, img, mask, centroid=None):
        ...
        for a in self.augmentations:
            img, mask = a(img, mask) if centroid is None else a(img, mask, centroid)

        return np.array(img, np.float32) / 255, np.array(mask, np.uint8)
